<div class="w-fill block" x-data="{ titleFocused: @entangle('titleFocused')}">
    <form wire:submit.prevent="saveProse">
        ...
        <div class="flex flex-row flex-wrap -mx-4 items-stretch">
            ...
            <div class="w-full px-4 py-2">
                ...
                <input type="text" id="title" wire:model="title" class="w-full border-px border-gray-300 border-solid bg-white py-2 px-3 rounded-md shadow-sm min-h-[42px] placeholder:text-gray-500 text-black font-normal text-base leading-tight focus:border-admin-main !ring-transparent disabled:text-black disabled:bg-gray-50 disabled:border-gray-300" @focus="titleFocused = true" @click.away="titleFocused = false">
                @error('title') <span class="text-red-600 text-sm block pt-0.5">{{ $message }}</span> @enderror
            </div>

            <div class="w-full px-4 py-2">
                <label class="block text-neutral-800 font-medium text-base mb-1" for="content">Content</label>
                <textarea type="text" id="content" wire:model="content" class="w-full border-px border-gray-300 border-solid bg-white py-2 px-3 rounded-md shadow-sm min-h-[42px] placeholder:text-gray-500 text-black font-normal text-base leading-tight focus:border-admin-main !ring-transparent disabled:text-black disabled:bg-gray-50 disabled:border-gray-300"></textarea>
                @error('content') <span class="text-red-600 text-sm block pt-0.5">{{ $message }}</span> @enderror
            </div>
            ...
            <div class="w-full px-4 py-2">
                <div class="flex flex-row justify-between items-center w-full pt-2">
                    <div>
                        <button type="submit" class="inline-block text-center border border-admin-main rounded-md min-h-[42px] h-auto py-2.5 p-5 text-white bg-admin-main text-base font-medium leading-tight hover:bg-admin-hover hover:border-admin-hover transition ease-in-out duration-200">Save Prose</button>
                    </div>

                    <div>
                        <a href="{{ route('prose.adminIndex') }}" class="inline-block text-center border border-admin-main rounded-md min-h-[42px] h-auto py-2.5 p-5 text-admin-main bg-white text-base font-medium leading-tight hover:bg-admin-hover hover:border-admin-hover hover:text-white transition ease-in-out duration-200">Cancel</a>
                    </div>
                </div>
            </div>
        </div>
