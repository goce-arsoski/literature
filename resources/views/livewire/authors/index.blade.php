<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-start sm:justify-between mb-2">
        ...
        <div>
            <div class="flex flex-row -mx-2 items-center">
                <div class="px-2 order-last sm:order-first"><label for="per_page_selected" class="text-sm text-gray-500">Results per page</label></div>
        ...
        <table class="min-w-full">
            <thead class="border-b bg-admin-main text-white">
                <tr>
                    ...
                    <th scope="col" class="text-sm sm:text-base px-2 py-2 text-center text-3.5 font-semibold">Role</th>
                ...
                <tr class="border-b">
                    <td class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 align-middle text-center">
                        <div class="inline-block rounded-full w-9 h-9">
                            @if($author->profile_photo_path)
                                <img src="{{ asset("storage/" . $author->profile_photo_path) }}" alt="" class="object-fit object-center w-full h-full rounded-full">
                            @else
                                <img src="{{ $author->profile_photo_url }}" alt="" class="object-fit object-center w-full h-full rounded-full">
                            @endif
                        </div>
                    </td>
                    <td class="text-sm md:text-base text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap align-middle text-center">{{ $author->id }}</td>
                    <td class="text-sm md:text-base text-neutral-800 font-medium text-4 px-2 py-2 whitespace-nowrap align-middle">{{ $author->name }}</td>
                    <td class="text-sm md:text-base text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap align-middle">{{ $author->email }}</td>
                    {{-- <td class="text-sm md:text-base text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap align-middle text-center">{{ ucfirst($author->getRoleNames()->implode(', ')) }}</td> --}}
                    <td class="text-sm text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap text-center align-middle">
                        <div class="w-auto inline-block relative z-10" x-data="{open:false, active:false}" x-bind:class="{'z-[60]' : active}">
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-100 bg-white" x-transition:enter-start="transform -translate-y-6 opacity-0" x-transition:enter-end="transform translate-y-0 opacity-100" x-transition:leave="transition ease-in duration-100 bg-transparent" x-transition:leave-start="transform translate-y-0 opacity-100" x-transition:leave-end="transform -translate-y-6 opacity-0" @click.away="open = false" class="absolute @if(count($authors) >= 5 && $loop->iteration >= count($authors) - 2) bottom-full @else top-full @endif right-0 bg-white shadow-lg min-w-full w-[110px] rounded-lg p-2" style="display: none;">

                                <ul class="mb-0">
                                    <li class="pb-1">
                                        <a href="{{ route('user.show', $author) }}" class="text-sm text-gray-500 py-1 px-2 block text-left bg-transparent font-normal transition-all duration-300 rounded-md hover:bg-blue-50 hover:text-admin-main hover:font-bold">View user</a>
                                    </li>
                                    <li class="pb-1">
                                        <a href="{{ route('user.edit', $author) }}" class="text-sm text-gray-500 py-1 px-2 block text-left bg-transparent font-normal transition-all duration-300 rounded-md hover:bg-blue-50 hover:text-admin-main hover:font-bold">Edit user</a>
                                    </li>
                                    <li class="pb-0">
                                        <a href="" wire:click.prevent="delete_user({{ $author }})" class="text-sm text-gray-500 py-1 px-2 block text-left bg-transparent font-normal transition-all duration-300 rounded-md hover:bg-blue-50 hover:text-admin-main hover:font-bold">Delete user</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
    ...
    {{-- <div>{{ $authors->onEachSide(2)->links('custom-pagination-links-view') }}</div> --}}
</div>
