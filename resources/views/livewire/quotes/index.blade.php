<div class="w-full">
        ...
        <table class="min-w-full">
            <thead class="border-b bg-admin-main text-white">
                <tr>
                    ...
                    <th scope="col" class="text-sm sm:text-base px-2 py-2 text-center text-3.5 font-semibold">Published</th>
                    <th scope="col" class="text-sm sm:text-base px-2 py-2 text-center text-3.5 font-semibold">Use global keywords<br>and description</th>
                    <th scope="col" class="text-sm sm:text-base px-2 py-2 text-left text-3.5 font-semibold">
                        Keywords
            ...
            <tbody>
                ...
                <tr class="border-b">
                    <td class="text-sm md:text-base text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap align-middle text-center">{{ $quote->id }}</td>
                    <td class="text-sm md:text-base text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap align-middle">{{ Str::words($quote->quote, 5, '... ') }}</td>
                    <td class="text-sm md:text-base text-neutral-800 font-medium text-4 px-2 py-2 whitespace-nowrap align-middle text-center">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" {{ $quote->published ? 'checked' : '' }} class="sr-only peer" wire:click="set_published({{ $quote->id }})">
                    ...
                    <td class="text-sm md:text-base text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap align-middle text-center">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" {{ $quote->use_global ? 'checked' : '' }} class="sr-only peer" wire:click="set_use_global({{ $quote->id }})">
                            <div class="w-11 h-6 bg-red-500 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        </label>
                    </td>
                    <td class="text-sm md:text-base text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap align-middle">{{ Str::words($quote->keywords, 5,'... ') }}</td>
                    <td class="text-sm md:text-base text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap align-middle">{{ Str::words($quote->description, 5,'... ') }}</td>
                    <td class="text-sm text-neutral-800 font-normal text-4 px-2 py-2 whitespace-nowrap text-center align-middle">
                        <div class="w-auto inline-block relative z-10" x-data="{open:false, active:false}" x-bind:class="{'z-[60]' : active}">
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-100 bg-white" x-transition:enter-start="transform -translate-y-6 opacity-0" x-transition:enter-end="transform translate-y-0 opacity-100" x-transition:leave="transition ease-in duration-100 bg-transparent" x-transition:leave-start="transform translate-y-0 opacity-100" x-transition:leave-end="transform -translate-y-6 opacity-0" @click.away="open = false" class="absolute  @if(count($quotes) >= 5 && $loop->iteration >= count($quotes) - 2) bottom-full @else top-full @endif right-0 bg-white shadow-lg min-w-full w-[110px] rounded-lg p-2" style="display: none;">

                                <ul class="mb-0">
                                    <li class="pb-1">
                                        <a href="{{ route('blog.show', $quote) }}" class="text-sm text-gray-500 py-1 px-2 block text-left bg-transparent font-normal transition-all duration-300 rounded-md hover:bg-blue-50 hover:text-admin-main hover:font-bold">View blog</a>
                                    </li>
                                    <li class="pb-1">
                                        <a href="{{ route('blog.edit', $quote) }}" class="text-sm text-gray-500 py-1 px-2 block text-left bg-transparent font-normal transition-all duration-300 rounded-md hover:bg-blue-50 hover:text-admin-main hover:font-bold">Edit blog</a>
                                    </li>
                                    <li class="pb-0">
                                        <a href="" wire:click.prevent="delete_blog({{ $quote }})" class="text-sm text-gray-500 py-1 px-2 block text-left bg-transparent font-normal transition-all duration-300 rounded-md hover:bg-blue-50 hover:text-admin-main hover:font-bold">Delete blog</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
    ...
    {{-- <div>{{ $quotes->onEachSide(3)->links('custom-pagination-links-view') }}</div> --}}
